...
def update_user_voted(quotes)
  quotes.map do |quote|
    quote.update_user_voted(@netid)
  end
  quotes
end
...
get '/quotes' do
  ...
  ResponseFormat.data(update_user_voted(quotes))
end
...
post '/quotes/:id/vote' do
  ...
  quote.votes.create(netid: @netid)
...
post '/quotes' do
  ...
  status, error = Quote.validate(params, %i[author source text])
  halt status, ResponseFormat.error(error) if error
